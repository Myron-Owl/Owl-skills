---
name: 1st-pr
description: 当常规方案反复失效、关键假设影响决策，或用户要求深入拆解复杂问题时，运用第一性原理分析事实、约束与替代方案。简单明确的任务直接处理，不启动完整分析流程。
---

## When to invoke this skill

简单问题直接回答；仅在关键假设影响方案、常规方法失效或用户要求深入拆解时使用完整分析流程。分析支持本次任务，不自动扩展为架构重构或多轮访谈。
...
3. **比较有意义的替代方案**：确有不同取舍时提供多个方向；约束已决定做法时给出该做法及依据，不为凑数量发明方案。
...
按任务复杂度选用下列阶段；已知背景和已确定结论直接复用，不要求每个阶段都单独输出或等待回复。

### Phase 1: 问题定义

先从当前请求与已有材料确定问题。以下问题只用于填补会改变方案的关键缺项，不逐项重复询问：
...
只指出有证据且会影响行动的假设或盲点，不以制造震撼感为目标。

### Phase 3: 重建方案

根据基本元素构建新的解决方案：
1. 提出推荐方案；存在有意义的取舍时再给出替代方案
...
完整分析可参考以下结构；小问题只输出结论、关键依据和下一步，不为套模板补齐无关章节：
...
3. **区分硬约束与待核验判断** — 未被证明不可能，不等于现实可行
4. **检查惯例的适用条件** — 有充分依据时采用成熟做法，不为创新而从零重建
5. **简单是终极复杂** — 基于基本原理的方案往往更简单
6. **依据可复核** — 给出关键事实、取舍理由和必要的计算或验证步骤
